...
#else
struct lzma_stream_stub {
  uint32_t avail_in = 0;
  uint32_t avail_out = 0;
  uint64_t total_out = 0;
  char *next_in = nullptr;
  char *next_out = nullptr;
};
template <uint32_t flags = 0>
struct lzma_tag_t {
  using state_type = lzma_stream_stub;
  using in_char_type = char;
  using out_char_type = char;
  using deflate_state_type = std::unique_ptr<state_type>;
  using inflate_state_type = std::unique_ptr<state_type>;
  using status_type = status_t;

  static status_type deflate(deflate_state_type&, bool) { return status_type::ERROR; }
  static status_type inflate(inflate_state_type&) { return status_type::ERROR; }
  static deflate_state_type new_deflate_state() { return std::make_unique<state_type>(); }
  static inflate_state_type new_inflate_state() { return std::make_unique<state_type>(); }
};
#endif
} // namespace decomp_tags
